...
namespace MilkInstancer
{
    ...
    public class MilkInstancer : MonoBehaviour
    {
        ...
        private void DrawInstances()
        {
            for (int i = 0; i < indirectMeshes.Length; i++)
            {
                ...
                {
                    Graphics.DrawMeshInstancedIndirect(irm.mesh, 0, irm.material[irm.lod2Material], m_bounds, m_instancesArgsBuffer, argsIndex + ARGS_BYTE_SIZE_PER_DRAW_CALL * 2, irm.lod00MatPropBlock, ShadowCastingMode.Off);
                    Graphics.DrawMeshInstancedIndirect(irm.mesh, 0, irm.material[irm.lod2Material], m_bounds, m_instancesArgsBuffer, argsIndex + ARGS_BYTE_SIZE_PER_DRAW_CALL * 2, irm.lod01MatPropBlock, ShadowCastingMode.Off);
                }
    ...
    class InstancingEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (!comp.TryGetComponent<RenderPipelineSetup>(out RenderPipelineSetup SU))
            {
                EditorGUILayout.Space(30);
                EditorGUILayout.BeginVertical();
                GUIStyle red = new GUIStyle();
                red.normal.textColor = Color.red;
                EditorGUILayout.LabelField("You do not have a Render Pipeline Setup component.", red);
                EditorGUILayout.LabelField("A depth texture will *not* be found (Occlusion Culling wont work),", red);
                EditorGUILayout.LabelField("And shadow distance will not be able to be obtained.", red);
